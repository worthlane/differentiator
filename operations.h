...
DEF_OP(DEG, "^", 2, 2, (pow(NUMBER_1, NUMBER_2)), "**", LatexOperationTypes::INFIX, "^", false, false, true, true,
{
    ...
    {
        return _MUL(d(node->left), _MUL(CPY(node->right), _DEG(CPY(node->left), _SUB(CPY(node->right), NUM(1)))));
    }
...
{
    return _MUL(NUM(-1), _MUL(d(node->right), _DIV(NUM(1), _DEG(_SIN(CPY(node->right)), NUM(2)))));
})
...
{
    return _MUL(d(node->right), _DIV(NUM(1), _DEG(_COS(CPY(node->right)), NUM(2))));
})
